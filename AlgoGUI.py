...
class Gui(Frame):
	...
	def __init__(self, algorithm=None, root=None):
		...
		self.stocks = None
		self.stats = None
		self.plotres = StringVar(); self.plotres.set('minute')
...
class Stats(Text):
	...
	def update(self):
		...
		self.insert( END, 'Return: ' + str(round(self.source.value/self.source.startingcapital-1,2) if self.source.startingcapital != 0 else 0) + '\n' )
...
class Stocks(Text):
	...
	def update(self):
		...
		for stock, amount in self.source.stocks.iteritems():
			if isinstance(self.source, alg.Backtester):
				self.insert( END, str(stock) + ':  ' + str(int(amount)) + '  $' + str(self.source.history(stock,interval=self.source.logging)[0].item()) + '\n' )
			else:
...
class Attributes(Text):
	...
	dontshow = set(['minutesago','daysago','logging','chartday','chartdaytimes','chartminute','running','benchmark', \
...
class Graph(FigureCanvasTkAgg):
	...
	def plotbenchmark(self,algo):
		if isinstance(algo, alg.Backtester) and algo.benchmark is not None:
			benchmark = algo.history(algo.benchmark,interval=algo.logging,length=len(algo.chartdaytimes if algo.logging == 'daily' else algo.chartminutetimes))
