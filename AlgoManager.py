...
class Algorithm(object):
	...
	def macd(self,stock,interval='daily',length=1,fastmawindow=12,slowmawindow=26,signalmawindow=9,fastmatype=1,slowmatype=1,signalmatype=1):
		...
		return {'macd' : md['MACD'][-length:], 'signal' : md['MACD_Signal'][-length:], 'macd hist': md['MACD_Hist'][-length:]}
	...
	def bollinger(self,stock,interval='daily',length=1,nbdevup=2,nbdevdn=2,matype=1,mawindow=20):
		bb, _ = tech.get_bbands(stock,interval=interval,nbdevup=nbdevup,nbdevdn=nbdevdn,matype=matype,time_period=mawindow)
		return {'top' : bb['Real Upper Band'][-length:], 'bottom' : bb['Real Lower Band'][-length:], 'middle' : bb['Real Middle Band'][-length:]}
...
class Backtester(Algorithm):
	...
	def start(self,startdate=datetime.datetime.today().date()-datetime.timedelta(days=10),enddate=datetime.datetime.today().date(),sleeptime=0):
	...
	def macd(self,stock,interval='daily',length=1,fastmawindow=12,slowmawindow=26,signalmawindow=9,fastmatype=1,slowmatype=1,signalmatype=1):
		name = 'macd,'+stock+','+interval+','+str(fastmawindow)+','+str(slowmawindow)+','+str(signalmawindow)+','+str(fastmatype)+','+str(slowmatype)+','+str(signalmatype)
		if name in self.storeddata:
		...
		return {'macd' : md['MACD'][len(md['MACD'])-self.timetoticks(interval)-length:len(md['MACD'])-self.timetoticks(interval)], 'signal' : md['MACD_Signal'][len(md['MACD'])-self.timetoticks(interval)-length:len(md['MACD'])-self.timetoticks(interval)], 'macd hist': md['MACD_Hist'][len(md['MACD'])-self.timetoticks(interval)-length:len(md['MACD'])-self.timetoticks(interval)]}


	def bollinger(self,stock,interval='daily',length=1,nbdevup=2,nbdevdn=2,matype=1,mawindow=20):
		name = 'bollinger,'+stock+','+interval+','+str(nbdevup)+','+str(nbdevdn)+','+str(matype)+','+str(mawindow)
		if name in self.storeddata:
			bb = self.storeddata[name]
		else:
			bb, _ = tech.get_bbands(stock,interval=interval,nbdevup=nbdevup,nbdevdn=nbdevdn,matype=matype,time_period=mawindow)
			self.storeddata[name] = bb
		return {'top' : bb['Real Upper Band'][len(bb['Real Upper Band'])-self.timetoticks(interval)-length:len(bb['Real Upper Band'])-self.timetoticks(interval)], 'bottom' : bb['Real Lower Band'][len(bb['Real Lower Band'])-self.timetoticks(interval)-length:len(bb['Real Lower Band'])-self.timetoticks(interval)], 'middle' : bb['Real Middle Band'][len(bb['Real Middle Band'])-self.timetoticks(interval)-length:len(bb['Real Middle Band'])-self.timetoticks(interval)]}
